...
// .wrangler/tmp/bundle-YLyybJ/checked-fetch.js
...
// src/session.ts
var SharedKnowledgeSession = class _SharedKnowledgeSession {
  static {
    __name(this, "SharedKnowledgeSession");
  }
  state;
  env;
  sessionKey;
  ...
  async fetch(request) {
    ...
    if (request.method === "POST" && url.pathname === "/submit") {
      const raw2 = await request.json();
      const parsed = messageSchema.safeParse(raw2);
      if (!parsed.success) {
      ...
      if (updated.length % _SharedKnowledgeSession.SUMMARY_INTERVAL === 0) {
...
// .wrangler/tmp/bundle-YLyybJ/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-YLyybJ/middleware-loader.entry.ts
